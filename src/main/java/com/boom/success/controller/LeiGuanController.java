...
public class LeiGuanController {
     ...
     * @param fixCode 固定码
     * @param childCode 发码
     * @param status 状态 @see com.boom.success.consts.LeiGuanStatusEnums
     * @param pageNo 页码
     * @param pageSize 页面大小
     * @return
     */
    ...
    public Result<Integer> batch(@RequestBody LeiGuanBatchRequest request, HttpServletRequest servletRequest) {
        ...
        LeiGuanStatusEnums statusEnums = LeiGuanStatusEnums.get(request.getOptType());
        if (statusEnums == null) {
        ...
        if (request.getOptType().intValue() == LeiGuanStatusEnums.INIT.getCode() && StringUtils.isEmpty(request.getKeeper())) {
            return Result.fail(GeneralCode.Param_Error.getCode(), "入库必须填写保管人");
        }
        if ((request.getOptType().intValue() == LeiGuanStatusEnums.ON_GOING.getCode()
                ||request.getOptType().intValue() == LeiGuanStatusEnums.ON_GOING.getCode())
